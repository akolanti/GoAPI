...
var ollamaClient *llmClient
var once sync.Once

func GetOllamaClient(ctx context.Context, modelName string, apikey string) llm.Provider {
	once.Do(func() {
		logger = logger_i.NewLogger("llm_ollama")
		newOllamaClient(ctx, modelName, apikey)
	})
	...
	return &llmClient{
		...
		modelName: ollamaClient.modelName,
		prompt:    ollamaClient.prompt,
	}
}

func newOllamaClient(ctx context.Context, modelName string, apikey string) {
	c := openai.NewClient(
		option.WithAPIKey(apikey),
		option.WithBaseURL(config.OllamaBaseURL),
	)

	ollamaClient = &llmClient{client: &c, modelName: modelName, prompt: config.LLMPrompt}
	logger.Debug("Ollama ", modelName, " client created")
	logger.Info("Ollama client created")
	go closeClient(ctx, ollamaClient)
}
...
func (c *llmClient) Generate(ctx context.Context, userQuery string, matches []string, messageHistory []string) (string, error) {
	...
	if err != nil {
		logger.Error("Error generating content from Ollama:", "error", err)
		return "", err
	}
	...
	return "", fmt.Errorf("no content returned from Ollama")
}
...
func (c *llmClient) ChatWithTools(ctx context.Context, messages []llm.Message, tools []llm.Tool) (*llm.Response, error) {
	...
	if err != nil {
		logger.Error("Error calling Ollama ChatWithTools:", "error", err)
		return nil, err
	}
	...
	return fromOllamaResponse(completion.Choices[0]), nil
}

func toOllamaMessages(messages []llm.Message) []openai.ChatCompletionMessageParamUnion {
...
func fromOllamaResponse(choice openai.ChatCompletionChoice) *llm.Response {
